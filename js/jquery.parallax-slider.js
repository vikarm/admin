...
(function($) {
	var zParallaxSlider=function(initObject){
		// configurable options for initObject
		this.slideDuration=2000; // the length of time to wait on a slide.
		this.fadeInDuration=1000; // the length of time for fading in a slide.
		this.fadeIn=true; // set to false to disable fade in of next slide.
		this.transition='horizontal'; // valid values are "crossfade", "horizontal" or "vertical" for setting the way slides transition.
		this.autoplay=true; // set to false to disable automatically advancing to the next slide.
		this.allowInstantSlideChange=false; // set to true to ignore all slide transition delays and immediately change to next slide on button clicks.
		this.slideEndCallback=function(obj){}; // register a callback function to run after a slide has been shown for its full duration, but before it has faded out.
		this.slideBeginCallback=function(obj){};  // register a callback function to run immediately before a slide has begun to appear.
		this.slideIndex=0; // set which slide should appear first
		this.direction="forward"; // valid values are "forward" and "back".  "back" will reverse the order of slides when going to next and previous slides.
		this.prev=null; // register a click event on a button for transitioning to the previous slide
		this.next=null; // register a click event on a button for transitioning to the next slide
		this.debug=false; // output verbose console log information for debugging purposes

		// defaults for zPSlide elements
		this.defaultAnimatedBackgroundDuration=3000; // The length of time that the background will animate.
		this.defaultSlideInDuration=3000; // length of time for the beginning slide transition
		this.defaultSlideOutDuration=1000; // length of time for the ending slide transition

		 // valid values for easing features depend on whether jquery easing plugin is loaded.  
		 // Refer to jquery easing plugin for more documentation: https://github.com/gdsmith/jquery.easing
		this.defaultAnimatedBackgroundEasing="linear"; // the easing used for background-position animation
		this.defaultSlideEasing="swing"; // the easing used for beginning slide transition
		this.defaultSlideOutEasing="swing"; // the easing used for ending slide transition

		// defaults for zPSlideElement elements
		this.defaultSlideElementInDuration=400; // the easing used for beginning slide element transition
		this.defaultSlideElementOutDuration=2000; // the easing used for ending slide element transition
		this.defaultSlideElementInDelay=400; // set a delay before a slide element begins its "in" animation
		this.defaultSlideElementOutDelay=0; // set a delay before a slide element begins its "out" animation
		this.defaultSlideElementInEasing="swing"; // the easing used for the slide element "in" animation
		this.defaultSlideElementOutEasing="swing"; // the easing used for the slide element "out" animation

		// for internal use only - don't try to configure these
		this.arrSlideObj=[];
		...
		this.checkButtonClick=function(){
			...
			if(!this.allowInstantSlideChange && (this.slideAnimatingOut || this.slideAnimating)){
				return;
			}
			if(this.doNextSlide){
		...
		this.nextSlideButton=function(){
			...
			if(this.allowInstantSlideChange || !this.slideAnimating){
		...
		this.setupSlideChangeTimeout=function(obj){
			var curObj=this;
			if(!this.allowInstantSlideChange){
				if(this.slideAnimating || this.slideAnimatingOut){
					setTimeout(function(){curObj.setupSlideChangeTimeout(obj);}, 100);
					return;
				}
			}
			var slideIndex=parseInt(obj.getAttribute("data-slide-index"))-1;
			var index=this.slideIndex;
			this.slideIndex=slideIndex;
			this.setupSlideOut(index);
		}
		this.init=function(obj){
			...
			$(this.container).each(function(){
				...
				if(typeof obj.setButtons != "undefined"){
					for(var i=0;i<obj.setButtons.length;i++){
						...
						$(s).bind("click", function(){
							this.PSlide.setupSlideChangeTimeout(this);
							return false;
						});
					}
				}
				for(var i2=0;i2<d.length;i2++){
					...
					d[i2].zPSlideConfig=curObj.setSlideConfig(slideConfig);
					...
					for(var i=0;i<arrSlideElements.length;i++){
						...
						s.zPSlideConfig=curObj.setSlideElementConfig(d[i2], s.zPSlideConfig);
						if(s.style.opacity){
					...
					curSlideIndex=i2;
					var arrBg=$(".zPSlideAnimatedBackground", d[i2]);
					var curYOffset=0;
					var curHeight=0;
						...
						n2.each(function(){
							...
							if(c.animateX != "0px" || c.animateY != "0px"){
								...
								if(c.type=='transition'){
									d[i2].arrAnimateBackgroundOut.push(this);
								}else{
									if(c.animateX != "0px"){
